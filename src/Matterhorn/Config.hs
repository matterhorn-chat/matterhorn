...
defaultBindings =
    [ (VtyRefreshEvent                  , [ ctrl (char 'l') ])
    , (ShowHelpEvent                    , [ fn 1 ])
    , (EnterSelectModeEvent             , [ ctrl (char 's') ])
    , (ReplyRecentEvent                 , [ ctrl (char 'r') ])
    , (ToggleMessagePreviewEvent        , [ meta (char 'p') ])
    , (InvokeEditorEvent                , [ meta (char 'k') ])
    , (EnterFastSelectModeEvent         , [ ctrl (char 'g') ])
    , (QuitEvent                        , [ ctrl (char 'q') ])
    , (NextChannelEvent                 , [ ctrl (char 'n') ])
    , (PrevChannelEvent                 , [ ctrl (char 'p') ])
    , (NextChannelEventAlternate        , [ kb Vty.KDown ])
    , (PrevChannelEventAlternate        , [ kb Vty.KUp ])
    , (NextUnreadChannelEvent           , [ meta (char 'a') ])
    , (ShowAttachmentListEvent          , [ ctrl (char 'x') ])
    , (ChangeMessageEditorFocus         , [ meta (char 'o') ])
    , (NextUnreadUserOrChannelEvent     , [ ])
    , (LastChannelEvent                 , [ meta (char 's') ])
    , (EnterOpenURLModeEvent            , [ ctrl (char 'o') ])
    , (ClearUnreadEvent                 , [ meta (char 'l') ])
    , (ToggleMultiLineEvent             , [ meta (char 'e') ])
    , (EnterFlaggedPostsEvent           , [ meta (char '8') ])
    , (ToggleChannelListVisibleEvent    , [ fn 2 ])
    , (ToggleExpandedChannelTopicsEvent , [ fn 3 ])
    , (CycleChannelListSorting          , [ fn 4 ])
    , (SelectNextTabEvent               , [ char '\t' ])
    , (SelectPreviousTabEvent           , [ kb Vty.KBackTab ])
    , (SaveAttachmentEvent              , [ char 's' ])
    , (LoadMoreEvent                    , [ ctrl (char 'b') ])
    ...
    , (ScrollTopEvent                   , [ kb Vty.KHome, meta $ char '<' ])
    , (ScrollBottomEvent                , [ kb Vty.KEnd, meta $ char '>' ])
    , (SelectOldestMessageEvent         , [ shift (kb Vty.KHome) ])
    , (SelectUpEvent                    , [ char 'k', kb Vty.KUp ])
    , (SelectDownEvent                  , [ char 'j', kb Vty.KDown ])
    , (ActivateListItemEvent            , [ kb Vty.KEnter ])
    , (SearchSelectUpEvent              , [ ctrl (char 'p'), kb Vty.KUp ])
    , (SearchSelectDownEvent            , [ ctrl (char 'n'), kb Vty.KDown ])
    , (ViewMessageEvent                 , [ char 'v' ])
    , (FillGapEvent                     , [ kb Vty.KEnter ])
    , (CopyPostLinkEvent                , [ char 'l' ])
    , (FlagMessageEvent                 , [ char 'f' ])
    , (OpenThreadEvent                  , [ char 't' ])
    , (PinMessageEvent                  , [ char 'p' ])
    , (YankMessageEvent                 , [ char 'y' ])
    , (YankWholeMessageEvent            , [ char 'Y' ])
    , (DeleteMessageEvent               , [ char 'd' ])
    , (EditMessageEvent                 , [ char 'e' ])
    , (ReplyMessageEvent                , [ char 'r' ])
    , (ReactToMessageEvent              , [ char 'a' ])
    , (OpenMessageURLEvent              , [ char 'o' ])
    , (AttachmentListAddEvent           , [ char 'a' ])
    , (AttachmentListDeleteEvent        , [ char 'd' ])
    , (AttachmentOpenEvent              , [ char 'o' ])
    , (CancelEvent                      , [ kb Vty.KEsc, ctrl (char 'c') ])
    , (EditorBolEvent                   , [ ctrl (char 'a') ])
    , (EditorEolEvent                   , [ ctrl (char 'e') ])
    , (EditorTransposeCharsEvent        , [ ctrl (char 't') ])
    , (EditorDeleteCharacter            , [ ctrl (char 'd') ])
    , (EditorKillToBolEvent             , [ ctrl (char 'u') ])
    , (EditorKillToEolEvent             , [ ctrl (char 'k') ])
    , (EditorPrevCharEvent              , [ ctrl (char 'b') ])
    , (EditorNextCharEvent              , [ ctrl (char 'f') ])
    , (EditorPrevWordEvent              , [ meta (char 'b') ])
    , (EditorNextWordEvent              , [ meta (char 'f') ])
    , (EditorDeleteNextWordEvent        , [ meta (char 'd') ])
    , (EditorDeletePrevWordEvent        , [ ctrl (char 'w'), meta (kb Vty.KBS) ])
    , (EditorHomeEvent                  , [ kb Vty.KHome ])
    , (EditorEndEvent                   , [ kb Vty.KEnd ])
    , (EditorYankEvent                  , [ ctrl (char 'y') ])
    , (FileBrowserBeginSearchEvent      , [ char '/' ])
    , (FileBrowserSelectEnterEvent      , [ kb Vty.KEnter ])
    , (FileBrowserSelectCurrentEvent    , [ kb (Vty.KChar ' ') ])
    , (FileBrowserListPageUpEvent       , [ ctrl (char 'b'), kb Vty.KPageUp ])
    , (FileBrowserListPageDownEvent     , [ ctrl (char 'f'), kb Vty.KPageDown ])
    , (FileBrowserListHalfPageUpEvent   , [ ctrl (char 'u') ])
    , (FileBrowserListHalfPageDownEvent , [ ctrl (char 'd') ])
    , (FileBrowserListTopEvent          , [ char 'g', kb Vty.KHome, meta $ char '<' ])
    , (FileBrowserListBottomEvent       , [ char 'G', kb Vty.KEnd, meta $ char '>' ])
    , (FileBrowserListNextEvent         , [ char 'j', ctrl (char 'n'), kb Vty.KDown ])
    , (FileBrowserListPrevEvent         , [ char 'k', ctrl (char 'p'), kb Vty.KUp ])
