...
defaultBindings =
    ...
    , (NextChannelEventAlternate        , [ key Vty.KDown ])
    , (PrevChannelEventAlternate        , [ key Vty.KUp ])
    ...
    , (SelectPreviousTabEvent           , [ key Vty.KBackTab ])
    , (SaveAttachmentEvent              , [ char 's' ])
    , (LoadMoreEvent                    , [ ctrl 'b' ])
    , (ScrollUpEvent                    , [ key Vty.KUp ])
    , (ScrollDownEvent                  , [ key Vty.KDown ])
    , (ScrollLeftEvent                  , [ key Vty.KLeft ])
    , (ScrollRightEvent                 , [ key Vty.KRight ])
    , (ChannelListScrollUpEvent         , [ ctrl Vty.KUp ])
    , (ChannelListScrollDownEvent       , [ ctrl Vty.KDown ])
    , (PageUpEvent                      , [ key Vty.KPageUp ])
    , (PageDownEvent                    , [ key Vty.KPageDown ])
    , (PageLeftEvent                    , [ shift Vty.KLeft ])
    , (PageRightEvent                   , [ shift Vty.KRight ])
    , (ScrollTopEvent                   , [ key Vty.KHome, meta '<' ])
    , (ScrollBottomEvent                , [ key Vty.KEnd, meta '>' ])
    , (SelectOldestMessageEvent         , [ shift Vty.KHome ])
    , (SelectUpEvent                    , [ char 'k', key Vty.KUp ])
    , (SelectDownEvent                  , [ char 'j', key Vty.KDown ])
    , (ActivateListItemEvent            , [ key Vty.KEnter ])
    , (SearchSelectUpEvent              , [ ctrl 'p', key Vty.KUp ])
    , (SearchSelectDownEvent            , [ ctrl 'n', key Vty.KDown ])
    , (ViewMessageEvent                 , [ char 'v' ])
    , (FillGapEvent                     , [ key Vty.KEnter ])
    ...
    , (CancelEvent                      , [ key Vty.KEsc, ctrl 'c' ])
    ...
    , (EditorHomeEvent                  , [ key Vty.KHome ])
    , (EditorEndEvent                   , [ key Vty.KEnd ])
    , (EditorYankEvent                  , [ ctrl 'y' ])
    , (FileBrowserBeginSearchEvent      , [ char '/' ])
    , (FileBrowserSelectEnterEvent      , [ key Vty.KEnter ])
    , (FileBrowserSelectCurrentEvent    , [ char ' ' ])
    , (FileBrowserListPageUpEvent       , [ ctrl 'b', key Vty.KPageUp ])
    , (FileBrowserListPageDownEvent     , [ ctrl 'f', key Vty.KPageDown ])
    , (FileBrowserListHalfPageUpEvent   , [ ctrl 'u' ])
    , (FileBrowserListHalfPageDownEvent , [ ctrl 'd' ])
    , (FileBrowserListTopEvent          , [ char 'g', key Vty.KHome, meta '<' ])
    , (FileBrowserListBottomEvent       , [ char 'G', key Vty.KEnd, meta '>' ])
    , (FileBrowserListNextEvent         , [ char 'j', ctrl 'n', key Vty.KDown ])
    , (FileBrowserListPrevEvent         , [ char 'k', ctrl 'p', key Vty.KUp ])
    , (FormSubmitEvent                  , [ key Vty.KEnter ])
